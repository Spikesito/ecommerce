 ...
 */
  ...
  {
    for ($i = 1; $i <= $nb; $i++) {
      $sql = "insert into invoices (CustomerId, ProductId,CommandNumber,Quantity) values (:CustomerId, :ProductId,:CommandNumber,:Quantity)";
      try {
        $sth = $dbh->prepare($sql);
        $sth->execute(array(":CustomerId" => random_int(1, $nbCustomerId), ":ProductId" => random_int(1, $nbProductId), ":CommandNumber" => $faker->uuid(), ":Quantity" => random_int(1, 200)));
      } catch (PDOException $e) {
        die("<p>Erreur lors de la requête SQL : " . $e->getMessage() . "</p>");
      }
    }
